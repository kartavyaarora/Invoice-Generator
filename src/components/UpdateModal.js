...
export default function UpdateModal(props) {
  ...
  const SaveInvoice = (data) => {
    ...
    html2canvas(document.querySelector("#invoiceCapture")).then((canvas) => {
      ...
      payload = Object.assign(data, { invoiceImage: imgData, items: props.items });
      props.info.invoiceNumber = props.number;
      let newList = [...invoiceList];
      for(let i=0; i<invoiceList.length;i++){
        if(invoiceList[i].invoiceNumber==props.id){
          newList[i]=payload;
        }  
      }
      dispatch({type:"EditInvoice",payload:newList})
      navigate('/')
    });
  }
